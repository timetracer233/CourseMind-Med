...
| 文本向量化 | sklearn TF-IDF | 本地轻量、稳定部署，满足 5 小时演示闭环 |
| 检索 | cosine similarity | 轻量，足够支撑黄金演示路径 |
| 图谱展示 | pyvis | 快速生成可交互 HTML 图谱 |
| 数据格式 | dataclass / Enum | 统一结构，便于表格展示和报告生成 |

**当前版本使用 TF-IDF 轻量向量化**：不依赖在线 Embedding API 和大模型下载，优先保证比赛现场稳定演示；后续可升级为 API embedding 或本地 sentence-transformers。
...
4. TF-IDF Vectorizer 生成轻量向量矩阵
5. 检索模块保存向量矩阵和 chunk 列表
...
| 在线 Embedding API 不可用 | 当前版本不依赖在线 Embedding API |
...
```bash
FAST_MODE_MAX_PAGES_PER_BOOK=60    # 默认只解析前 60 页
FAST_MODE_MAX_CHAPTERS_PER_BOOK=8  # 每本教材默认最多处理 8 个章节
```
